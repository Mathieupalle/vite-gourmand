...
<form method="post">
    ...
    <input type="number" step="0.01" name="prix_par_personne" required><br><br>

    <label>Nombre minimum de personnes</label><br>
    <input type="number" name="nombre_personne_minimum" required><br><br>

    <label>Quantité restante</label><br>
    <input type="number" name="quantite_restante"><br><br>

    <label>Thème</label><br>
    <select name="theme_id" required>
        <option value="">--</option>
        <?php foreach ($themes as $t): ?>
            <option value="<?= (int)$t['theme_id'] ?>"><?= htmlspecialchars((string)$t['libelle']) ?></option>
        <?php endforeach; ?>
    </select><br><br>
    ...
    <select name="regime_id" required>
        ...
        <?php foreach ($regimes as $r): ?>
            <option value="<?= (int)$r['regime_id'] ?>"><?= htmlspecialchars((string)$r['libelle']) ?></option>
        <?php endforeach; ?>
    </select><br><br>
