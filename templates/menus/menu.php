        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-2">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?= htmlspecialchars((string)$menu['titre'], ENT_QUOTES, 'UTF-8') ?></li>
                </ol>
            </nav>
            <h1 class="h3 mb-0"><?= htmlspecialchars((string)$menu['titre'], ENT_QUOTES, 'UTF-8') ?></h1>
        </div>
                            ...
                            <div class="carousel-inner rounded overflow-hidden border">
                                <?php foreach ($imagesWeb as $i => $src): ?>
                                    <div class="carousel-item <?= $i === 0 ? 'active' : '' ?>">
                                        <img src="<?= htmlspecialchars((string)$src, ENT_QUOTES, 'UTF-8') ?>" class="d-block w-100" alt="Image du menu">
                                    </div>
                                <?php endforeach; ?>
                            </div>
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-white d-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex gap-2">
                        <span class="badge text-bg-light border"><?= htmlspecialchars((string)$menu['theme'], ENT_QUOTES, 'UTF-8') ?></span>
                        <span class="badge text-bg-light border"><?= htmlspecialchars((string)$menu['regime'], ENT_QUOTES, 'UTF-8') ?></span>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <?php if (!empty($menu['description'])): ?>
                        <div class="mb-3">
                            ...
                            <div><?= nl2br(htmlspecialchars((string)$menu['description'], ENT_QUOTES, 'UTF-8')) ?></div>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($menu['conditions'])): ?>
                        <div class="alert alert-warning mb-3">
                            ...
                            <div><?= nl2br(htmlspecialchars((string)$menu['conditions'], ENT_QUOTES, 'UTF-8')) ?></div>
                        </div>
                    <?php endif; ?>
                            ...
                            <div class="accordion-body">
                                <?php if (!empty($menu['plats_grouped'][$catKey])): ?>
                                    <?php foreach ($menu['plats_grouped'][$catKey] as $p): ?>
                                        <div class="mb-2">
                                            <div class="fw-semibold"><?= htmlspecialchars($p['titre'], ENT_QUOTES, 'UTF-8') ?></div>
