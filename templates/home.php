...
<div class="py-5 hero">
    <div class="container py-2">
        <div class="row g-4">
            <div class="col-lg-7">
                <span class="badge bg-light text-dark mb-3">Traiteur en ligne • Bordeaux • 25 ans d’expérience</span>
                ...
                <div class="d-flex gap-2 flex-wrap">
                    <a class="btn btn-primary px-4" href="<?= BASE_URL ?>/menus">Voir les menus</a>
                    <a class="btn btn-outline-secondary px-4" href="<?= BASE_URL ?>/contact">Demander un devis</a>
                </div>
                <div class="small text-muted mt-3">
                    <?php if ($user && isset($user['prenom'])): ?>
                        Connecté en tant que <?= htmlspecialchars($user['prenom']); ?>.
                    <?php elseif ($user): ?>
                        Connecté.
                    <?php else: ?>
                        Créez un compte pour suivre vos commandes et laisser un avis.
                    <?php endif; ?>
                </div>
            </div>
            <div class="col-lg-5">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                <div class="p-4 bg-white">
                    ...
                    <a class="btn btn-sm btn-link px-0 mt-2" href="<?= $user ? BASE_URL.'/mesCommandes' : BASE_URL.'/login' ?>">
                        <?= $user ? 'Voir mes commandes →' : 'Se connecter →' ?>
                    </a>
                </div>
            </div>
            <div class="col-md-4">
                    ...
                    <ul class="list-group">
                        <?php foreach ($avisValides as $a): ?>
                            <li class="list-group-item d-flex gap-3 align-items-start">
                                <span class="badge rounded-pill mt-1 badge-brand"><?= (int)$a['note']; ?>/5</span>
                                <div>
                            ...
                            <?php foreach ($horaires as $h): ?>
                                <li class="d-flex justify-content-between py-2 border-bottom">
                                    ...
                                    <span class="text-muted">
                                        <?= !empty($h['heure_ouverture']) && !empty($h['heure_fermeture'])
